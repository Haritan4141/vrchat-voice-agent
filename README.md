...
現時点の実装は、`Windowsの音声ルーティング + faster-whisper + Ollama + VOICEVOX + 仮想マイク` です。`small` / `medium` / `large-v3` などは設定ファイルから後で変更できます。
...
`faster-whisper` は別途インストールしてください。

```powershell
python -m pip install faster-whisper
```

RTX 3090 環境の初期値は次の方針で十分です。

- `model = "small"`
- `device = "cuda"`
- `compute_type = "float16"`

精度を上げたい時は `model` を `medium` や `large-v3` に変更してください。
...
現段階のMVP本体は、音声I/OにWindows標準API、STTに `faster-whisper` を使います。
