...
操作画面の`約5秒の動作テスト`は、CABLE-Bの音量判定を一時的に迂回して、発話動作、頷き、7種類の表情をOSCへ直接送ります。これで動けばUnity/VRChat/OSC側は正常で、通常発話で動かない原因はCABLE-Bの検出値または閾値側です。テストは約5秒で通常表情・待機状態へ自動復帰します。
