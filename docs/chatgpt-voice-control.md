...
どちらもVRChatの標準OSC `/chatbox/input`へ送ります。UIAは最大144文字、STTは長い認識結果を句点・疑問符・読点の近くで区切り、既定では直近48文字以内の短いフレーズだけを表示します。区切りがない長文は先頭へ`…`を付けた末尾48文字になります。文字起こし処理には全文を使うため、この表示短縮でSTTの認識精度は変わりません。接頭辞は既定で付けません。`/chatbox/typing`もAI発話中だけONになります。チャットボックス表示はアバター改変を使わないため、この機能だけのための再アップロードは不要です。
...
1. VRChatのOSCとチャットボックス表示を有効にし、GUIの`字幕表示テスト`で`字幕表示テストです`が見えることを確認する
